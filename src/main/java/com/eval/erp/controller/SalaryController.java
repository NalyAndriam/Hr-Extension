...
public class SalaryController {
    ...
    public String getSalaryTotals(@RequestParam(required = false) String year, Model model) {
        ...
        try {
            ...
            logger.info("Fetched {} salary totals and chart data for year: {}", 
                        salaryTotals.size(), year != null ? year : "All");
        } catch (Exception e) {
            logger.error("Error fetching salary totals or chart data for year {}: {}", year, e.getMessage());
            model.addAttribute("error", e.getMessage());
        }
        return "pages/salary-total";
    }
